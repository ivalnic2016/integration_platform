...
- 🎯 Entry points to execute your program using `python -m <integration_platform>` or `$ integration_platform` with basic CLI argument parsing.
...
---
# integration_platform Flask Application

Awesome integration_platform created by ivalnic2016
...
```bash
git clone https://github.com/ivalnic2016/integration_platform integration_platform
...
```bash
pip install integration_platform
```
...
```bash
integration_platform create-db   # run once
integration_platform populate-db  # run once (optional)
integration_platform add-user -u admin -p 1234  # ads a user
integration_platform run
```
